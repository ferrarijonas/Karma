...
Fase 2: Triagem     (@gerir prepara tarefa)
Fase 3: Agir         (@construir → gate-runner)
Fase 4: Verificar    (@avaliar adversarial + @aprender consolida)
...
|--------|-------|-------------|
| **@gerir** | Cria, gerencia e despacha tarefas | `Task({ agent: "gerir", ... })` |
| **@construir** | Lê briefing, implementa, roda gate | `Task({ agent: "construir", ... })` |
| **@aprender** | Consolida memória pós-tarefa | `Task({ agent: "aprender", ... })` |
| **@planejar** | Modo seguro — só lê, pergunta, não executa | `Task({ agent: "planejar", ... })` |
